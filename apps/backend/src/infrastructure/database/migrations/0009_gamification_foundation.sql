/*
Tujuan: Menambah fondasi database gamifikasi untuk karakter murid, XP ledger, badge, dan quest.
Caller: Drizzle migrate dan workflow `bun run db:migrate`.
Dependensi: users, PostgreSQL enum, foreign key, unique constraint, dan index.
Main Functions: Membuat enum/tabel student_profiles, xp_ledger, badges, student_badges, quests, dan student_quests beserta index lookup utama.
Side Effects: Menambah objek schema gamifikasi ke database PostgreSQL.
*/

CREATE TYPE "gamification_character" AS ENUM('ASTRA', 'NOVA', 'LUMI', 'RAKA');--> statement-breakpoint
CREATE TYPE "gamification_event_type" AS ENUM('MATERIAL_COMPLETED', 'MODULE_COMPLETED', 'PRACTICE_COMPLETED', 'EXAM_SUBMITTED', 'EXAM_GRADED', 'PROCTOR_CLEAN_EXAM', 'DAILY_STREAK', 'QUEST_CLAIMED');--> statement-breakpoint
CREATE TYPE "badge_tier" AS ENUM('BRONZE', 'SILVER', 'GOLD', 'PLATINUM');--> statement-breakpoint
CREATE TYPE "quest_type" AS ENUM('DAILY', 'WEEKLY', 'EVENT');--> statement-breakpoint
CREATE TYPE "quest_status" AS ENUM('ACTIVE', 'COMPLETED', 'CLAIMED', 'EXPIRED');--> statement-breakpoint
...
CREATE INDEX "idx_xp_ledger_event_created" ON "xp_ledger" USING btree ("event_type","created_at");--> statement-breakpoint
...
CREATE INDEX "idx_student_quests_quest_status" ON "student_quests" USING btree ("quest_id","status");
