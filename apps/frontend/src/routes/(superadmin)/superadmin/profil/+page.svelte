<!--
Tujuan: Menyediakan halaman profil super admin fase 1.
Caller: Route `/superadmin/profil`.
Dependensi: ProfilePanel dan data user dari layout server.
Main Functions: Menampilkan form update profil dan password untuk super admin.
Side Effects: Tidak ada selain render komponen form profil.
-->
...
<div class="flex rounded-2xl border-4 border-black bg-white shadow-solid overflow-hidden mb-8">
  ...
  <div class="flex-grow p-6 flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
    <div>
      ...
      <h1 class="text-3xl font-black uppercase tracking-tight text-ink">Profil Super Admin</h1>
      <p class="mt-1 text-sm font-bold text-ink/60">Perbarui data akun super admin Anda dari halaman ini.</p>
    </div>
  </div>
</div>
